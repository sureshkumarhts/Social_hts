module ActivePollHelper
  ...
  def active_poll(name, opthash)
    ...
    if poll.blank?
      ...
      unless user_id.blank? && poll.target == TARGET_LOGGED_USER
        logged_user_has_voted = false || user_has_voted_poll?(poll, user_id) unless user_id.blank?
        #Ask if the conditions are met to show the poll
        if user_can_see_poll?(poll, user_id)
