...
class EventsController < ApplicationController
  ...
  def index
    @section = 'EVENTS'
    @events = Event.find(:all, :conditions=>'start_time>now() and group_id is NULL', :order=>'start_time ASC')
  ...
  def show
    if request.xhr?
      ...
      render :update do |page|
        page.replace_html  "group_event", :partial => 'events/event_show', :object => @event
        page.hide "group_event_brief"
      end
    else
@event = Event.find(params[:id])
    respond_to do |format|
  ...
  def create
    ...
    @event.start_at = params[:event][:start_time]
    @event.end_at = params[:event][:end_time]
  ...
  def update
     ...
     params[:event][:city]
     puts "ddddddddddddddddd",params[:event][:location] + params[:event][:city]
    @event = Event.find(params[:id])
    respond_to do |format|
