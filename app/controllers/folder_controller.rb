...
class FolderController < ApplicationController
 ...
 # before_filter :authorize_deleting, :only => :destroy
  ...
  def list
    ...
    #@can_update = current_user.can_update(@folder.id)
    #@can_delete = current_user.can_delete(@folder.id)
    ...
    # List of subfolders
    puts "sssssssssssssssssssssssssssssssss",current_user.inspect
    @folders = @folder.list_subfolders(current_user, folder_order.rstrip)

    # List of files in the folder
    @myfiles = @folder.list_files(current_userr, file_order.rstrip)
